...
import type { FormValues } from "./CreateEventFormProvider";
	...
	if (coverFile) {
		...
		if (coverFileData) {
			...
			await supabase
				.from("events")
				.update({
					cover: `https://${import.meta.env.VITE_PROJECT_ID}.supabase.co/storage/v1/object/public/events-assets/${
						coverFileData.path
					}`,
				})
				.eq("id", eventId);
	...
	if (iconFile) {
		...
		if (iconFileData) {
			...
			await supabase
				.from("events")
				.update({
					logo: `https://${import.meta.env.VITE_PROJECT_ID}.supabase.co/storage/v1/object/public/events-assets/${
						iconFileData.path
					}`,
				})
				.eq("id", eventId);
